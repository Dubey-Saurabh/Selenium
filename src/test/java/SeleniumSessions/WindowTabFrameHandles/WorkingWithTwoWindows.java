...
/*		17. driver.switchTo().window(driver.getWindowHandles().iterator().next()): Switches to the first window in the list of window handles.
		...
		19. driver.switchTo().window(driver.getWindowHandles().stream().filter(handle -> handle.equals(windowHandle)).findFirst().orElse(null)):
		      Switches to a specific window handle using a stream filter.
		20. driver.switchTo().window(driver.getWindowHandles().stream().reduce((first, second) -> second).orElse(null)):
		      Switches to the last opened window handle.
		21. driver.switchTo().window(driver.getWindowHandles().stream().findFirst().orElse(null)):
